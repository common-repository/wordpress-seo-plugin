...
function wds_seoopt_settings() {
	...
	if ( 'posts' == get_option('show_on_front') ) {
		$fields['home'] = array(
			'title' =>  __( 'Home' , 'wds' ),
			'intro' =>  '<div class="error below-h2"><p><a title="Upgrade Now" href="http://premium.wpmudev.org/project/wpmu-dev-seo">Upgrade to WPMU DEV SEO Pro to edit these values</a></p></div>',
			'options' => array(
				array(
					'type' => 'text_disabled',
					'name' => 'title-home',
	...
	foreach (get_post_types() as $posttype) {
		...
		if (isset($wds_options['redirectattachment']) && $wds_options['redirectattachment'] && $posttype == 'attachment')
			continue;
		$pt_title = preg_replace('/_+/', ' ', ucfirst($posttype));
		$fields[$posttype] = array(
			...
			'options' => array(
				array(
					'type' => 'text_disabled',
					'name' => 'title-' . $posttype,
					'title' => sprintf( __( '%s Title' , 'wds'), $pt_title ),
					'description' => ''
				),
				array(
					'type' => 'textarea_disabled',
					'name' => 'metadesc-' . $posttype,
					'title' => sprintf( __( '%s Meta Description' , 'wds'), $pt_title ),
					'description' => ''
				)
			)
		);
	...
	foreach (get_taxonomies() as $taxonomy) {
		if ( in_array($taxonomy, array( 'link_category','nav_menu', 'post_format' ) ) )
			continue;
		$tax_title = preg_replace('/_+/', ' ', ucfirst($taxonomy));
		$fields[$taxonomy] = array(
			...
			'options' => array(
				array(
					'type' => 'text_disabled',
					'name' => 'title-' . $taxonomy,
					'title' => sprintf( __( '%s Title' , 'wds'), $tax_title ),
					'description' => ''
				),
				array(
					'type' => 'textarea_disabled',
					'name' => 'metadesc-' . $taxonomy,
					'title' => sprintf( __( '%s Meta Description' , 'wds'), $tax_title ),
					'description' => ''
				)
			)
		);
